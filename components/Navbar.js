    ...
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-500 pt-safe">
      <div className="max-w-7xl mx-auto px-4 pt-2 md:pt-4">
        <div className="rounded-[1.5rem] glass-effect border border-[var(--border-light)] shadow-2xl transition-all duration-500 backdrop-blur-md">
          <div className="px-3 md:px-10 h-14 md:h-20 flex items-center justify-between gap-1">
            <a href="#hero" className="flex items-center gap-2 select-none group min-touch-target">
            ...
            <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-[var(--text-muted)]">
              {["#hero", "#catalog", "#about", "#contact"].map((href, i) => (
                <a
                  key={href}
                  href={href}
                  className="hover:text-[var(--text-primary)] transition duration-300 relative group py-3"
                >
            ...
            <div className="flex items-center gap-1.5 md:gap-3">
              <button
                ...
                className="hidden md:flex w-10 h-10 md:w-11 md:h-11 rounded-xl hover:bg-[var(--bg-subtle)] border border-[var(--border-subtle)] hover:border-gold/20 transition-all duration-300 text-[var(--text-muted)] hover:text-gold items-center justify-center active:scale-90 min-touch-target"
              ...
              <button
                ...
                className="w-10 h-10 md:w-11 md:h-11 rounded-xl hover:bg-[var(--bg-subtle)] border border-[var(--border-subtle)] hover:border-gold/20 transition-all duration-300 text-gold flex items-center justify-center active:scale-90 min-touch-target"
              ...
              <button
                ...
                className="w-10 h-10 md:w-11 md:h-11 rounded-xl hover:bg-[var(--bg-subtle)] border border-[var(--border-subtle)] hover:border-gold/20 transition-all duration-300 text-gold flex items-center justify-center font-sans font-bold text-[11px] md:text-xs active:scale-90 min-touch-target"
                aria-label="Switch Language"
              >
                {isEnglish ? "AR" : "EN"}
              </button>

              <div className="hidden md:flex items-center bg-[var(--bg-subtle)] border border-[var(--border-light)] rounded-xl px-4 h-11 focus-within:border-gold/30 transition-all duration-300">
                <input
                  ref={searchInputRef}
                  type="text"
                  value={searchQuery}
                  ...
                  className="bg-transparent border-none outline-none text-sm text-[var(--text-primary)] w-36 focus:w-48 transition-all duration-300"
                />
              ...
              <button
                onClick={onCartOpen}
                className={`relative w-10 h-10 md:w-11 md:h-11 rounded-xl hover:bg-[var(--bg-subtle)] border border-[var(--border-subtle)] hover:border-gold/20 transition-all duration-300 text-[var(--text-primary)] flex items-center justify-center active:scale-90 min-touch-target ${cartWobble ? "animate-wobble" : ""}`}
