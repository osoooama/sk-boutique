        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 order-2 lg:order-1 space-y-6 text-start">
            <span className="inline-flex items-center gap-1.5 bg-gold/10 text-gold border border-gold/25 px-3.5 py-1 rounded-full text-xs font-bold tracking-wide">
              {isEnglish ? "European Quality · Locally Crafted" : "جودة أوروبية · صناعة محلية"}
            </span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-[var(--text-primary)] leading-tight font-cairo">
              {isEnglish ? (
                <>Everything we have is beautiful <br /><span className="text-gold">European designs crafted locally with precision</span></>
              ) : (
                <>كل ما لدينا جميل <br /><span className="text-gold">تصاميم أوروبية تُصنع محلياً بدقة</span></>
              )}
            </h2>
            <p className="text-[var(--text-muted)] text-sm leading-relaxed font-light">
              {isEnglish
                ? "We take pride in blending European design with local craftsmanship. Sara Krishan Boutique is pleased to present this special collection using premium imported fabrics, crafted locally to meet EU quality standards."
                : "نعتز بدمج التصميم الأوروبي الأنيق مع الصناعة المحلية الحرفية. يسر متجر سارة كريشان تقديم هذه التشكيلة الخاصة بأفضل الخامات المستوردة، تُصنع محلياً بمعايير الجودة الأوروبية."}
            </p>
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  <span className="bg-gold text-black text-[10px] font-bold px-3 py-1 rounded-full">
                    {isEnglish ? "National Team Support 🇯🇴" : "دعم المنتخب 🇯🇴"}
                  </span>
                ...
                <div className="mt-4 space-y-1.5">
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    {isEnglish ? "On all items — supporting our national team" : "على جميع القطع دعماً للمنتخب الوطني"}
                  </p>
                </div>
              ...
              <div className="p-1 bg-gradient-to-b from-gold/10 to-transparent">
                <div className="aspect-video w-full relative rounded-2xl overflow-hidden bg-[var(--bg-primary)] border border-[var(--border-subtle)]">
                  <iframe
                    ...
                    title={isEnglish ? "European Quality - SK Boutique" : "جودة أوروبية - متجر سارة كريشان"}
              ...
              <div className="bg-[var(--bg-secondary)] px-6 py-4 flex items-center justify-between border-t border-[var(--border-subtle)] text-xs text-[var(--text-muted)]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-[var(--text-primary)]">
                    {isEnglish ? "Watch the European quality and precision of our locally crafted designs" : "شاهد جودة ودقة تصاميمنا الأوروبية المصنوعة محلياً"}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 text-gold">
                  <i className="fab fa-youtube text-red-600 text-sm animate-pulse" />
                  <span>{isEnglish ? "EU Quality · Locally Made" : "جودة أوروبية · صناعة محلية"}</span>
                </div>
              </div>
            </div>
          </div>
